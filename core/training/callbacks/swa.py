...
#             logger.info(
#                 f"Epoch {epoch + 1}: Updating SWA model Batch Normalization statistics..."
#             )
#             try:
#                 # Ensure the SWA model itself is on the correct device
#                 if self.device:
#                     self.swa_model.to(self.device)
#                 # Update BN stats using the validation loader
#                 torch.optim.swa_utils.update_bn(
#                     val_loader, self.swa_model, device=self.device
#                 )
...
#                     )
# core/training/callbacks/swa.py
...
class SWACallback(Callback):
    ...
    Args:
        **kwargs: Configuration parameters. Expected keys:
            ...
            anneal_strategy (str): 'cos' or 'linear'. Default: 'cos'.
            update_bn_epochs (int): How often to update BN stats using val loader during SWA. 0 disables. Default: 5.
            # device: Should be passed explicitly by the caller or set via context.
            # val_loader: Will be set via context.
            # train_loader: Will be set via context.
            # swa_model: Optional pre-created AveragedModel. If None, created internally.
    """
    ...
    def __init__(self, **kwargs):
        ...
        # External objects (will be set by set_trainer_context or on_train_begin)
        self.device = kwargs.get("device")  # Allow passing via config as fallback
        self._swa_model_external = kwargs.get("swa_model")  # Allow passing SWA model
        self.val_loader = None  # Initialize to None, will be set by Trainer
        self.train_loader = None  # Initialize to None, will be set by Trainer
        ...
        unused_keys = set(kwargs.keys()) - {
            ...
            "anneal_strategy",
            "update_bn_epochs",
            "device",  # device is handled
            "swa_model",  # swa_model is handled
        }
        if unused_keys:
            logger.warning(f"Unused config keys for SWACallback: {list(unused_keys)}")

    # Add set_trainer_context if needed for explicit context passing
    def set_trainer_context(self, context: Dict[str, Any]):
        """Receive context from the Trainer."""
        ...
        if self._optimizer is None:
            self._optimizer = context.get("optimizer")
        if self._total_epochs is None:
            self._total_epochs = context.get("total_epochs")

        logger.debug(
            f"SWACallback received context. val_loader: {self.val_loader is not None}, train_loader: {self.train_loader is not None}"
        )
    ...
    def on_train_begin(self, logs: Optional[Dict[str, Any]] = None) -> None:
        """Get necessary components if not already set."""
        logs = logs or {}
        # Primarily rely on set_trainer_context, but fallback to logs if needed
        if self._model is None:
            self._model = logs.get("model")
        if self._optimizer is None:
            self._optimizer = logs.get("optimizer")
        if self._total_epochs is None:
            self._total_epochs = logs.get("total_epochs")
        if self.device is None:
            self.device = logs.get("device")
        if self.val_loader is None:
            self.val_loader = logs.get("val_loader")
        if self.train_loader is None:
            self.train_loader = logs.get("train_loader")

        if self._model is None or self._optimizer is None:
            logger.error(
                "SWACallback requires 'model' and 'optimizer' to be set (via context or logs)."
            )
            return

        if self._total_epochs is None:
            logger.warning(
                "SWACallback: 'total_epochs' not set. Cannot determine SWA start epoch based on fraction."
            )
            if self.swa_start_frac > 0:
                logger.error("Disabling SWA because total_epochs is unknown.")
                self.swa_start_frac = 1.1  # Effectively disable
                self._swa_start_epoch = float("inf")
            else:
                self._swa_start_epoch = 0
        else:
            self._swa_start_epoch = int(self._total_epochs * self.swa_start_frac)

        if self.device is None:
            self.device = next(self._model.parameters()).device  # Infer from model
            logger.warning(f"SWACallback: Inferring device from model: {self.device}")
    ...
    def on_epoch_end(self, epoch: int, logs: Optional[Dict[str, Any]] = None) -> None:
        ...
        # --- Update Batch Normalization Statistics ---
        if self.update_bn_freq > 0 and (epoch + 1) % self.update_bn_freq == 0:
            # Use the stored val_loader or train_loader
            ...
            if loader_to_use is None:
                ...
                if loader_to_use is None:
                    logger.warning(
                        "SWACallback: Cannot update BN stats - neither val_loader nor train_loader is available."
                    )
                    return
                else:
                    logger.info(
                        f"SWACallback: Using {loader_name} for BN stats update as fallback."
                    )
            else:
                logger.info(f"SWACallback: Using {loader_name} for BN stats update.")
            ...
            logger.info(
                f"Epoch {epoch + 1}: Updating SWA model Batch Normalization statistics using {loader_name}..."
            )
            try:
                # Ensure the SWA model itself is on the correct device
                if self.device:
                    self.swa_model.to(self.device)
                # Update BN stats using the selected loader
                torch.optim.swa_utils.update_bn(
                    loader_to_use, self.swa_model, device=self.device
                )
                logger.info("  SWA BN statistics updated.")
            except Exception as e:
                logger.error(
    ...
    def on_train_end(self, logs: Optional[Dict[str, Any]] = None) -> None:
        ...
        if self._is_swa_active:
            ...
            if logs and "experiment_dir" in logs:
                exp_dir = logs["experiment_dir"]
            elif self.experiment_dir:  # Use stored experiment_dir
            ...
            if exp_dir:
                try:
                    ...
                    # Ensure model is on CPU before saving for better portability
                    swa_model_state = self.swa_model.module.state_dict()
                    cpu_state_dict = {k: v.cpu() for k, v in swa_model_state.items()}
            ...
            # Optionally copy SWA weights back to the original model if requested
            # Be cautious with this - might overwrite the best non-SWA weights
            copy_to_main = logs.get("swa_copy_to_main", False) if logs else False
            if copy_to_main and self._model and self.swa_model:
                try:
                    logger.info("Copying final SWA weights back to original model.")
