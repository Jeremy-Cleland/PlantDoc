...
# Assuming utility functions are in utils
...
class ModelCheckpoint(Callback):
    ...
    priority = (
        80  # Run after metric calculation but before LR scheduling/early stopping
    )
    ...
    def __init__(
        ...
        if not (
            ...
            or (isinstance(self.save_freq, int) and self.save_freq > 0)
        ):
            raise ValueError(
                f"save_freq must be 'epoch' or a positive integer, got {self.save_freq}"
            )
        ...
        try:
            ...
            logger.error(
                f"Failed to create checkpoint directory '{self.dirpath}': {e}",
                exc_info=True,
            )
    ...
    def _get_monitor_value(self, logs: Dict[str, Any]) -> Optional[float]:
        ...
        if value is None:
            if self.save_best_only:
                logger.warning(
                    f"ModelCheckpoint: Monitor '{self.monitor}' not found in logs: {list(logs.keys())}. Skipping best check."
                )
            return None
        elif not isinstance(value, numbers.Number):
            logger.warning(
                f"ModelCheckpoint: Monitor '{self.monitor}' is not a number ({type(value).__name__}). Skipping best check."
            )
            return None
        return float(value)
    ...
    def _save_checkpoint(
        ...
        if metric_value is not None:
            state[self.monitor] = metric_value  # Current value
        ...
        try:
            ...
            if self.verbose:
                logger.info(f"Checkpoint saved: '{filepath.name}' (Epoch {epoch + 1})")
        except Exception as e:
            logger.error(
                f"Failed to save checkpoint to '{filepath}': {e}", exc_info=True
            )
            if temp_filepath.exists():
                try:
                    temp_filepath.unlink()
                except OSError:
                    logger.error(f"Failed to remove temporary file: {temp_filepath}")
        finally:  # Ensure temp file cleanup on any exit path if rename failed
            if temp_filepath.exists():
                try:
                    temp_filepath.unlink()
                    logger.warning(
                        f"Removed incomplete temp checkpoint: {temp_filepath}"
                    )
                except OSError:
                    pass
    ...
    def on_epoch_end(self, epoch: int, logs: Optional[Dict[str, Any]] = None) -> None:
        ...
        if not model:
            logger.error("ModelCheckpoint: 'model' not found. Cannot save.")
            return
        if self.save_optimizer and not optimizer:
            logger.warning(
                "ModelCheckpoint: 'optimizer' not found. Optimizer state will not be saved."
            )
        ...
        filepath_dict = {
            **logs,
            "epoch": epoch + 1,
        }  # Use 1-based epoch for filename formatting
        save_occurred = False
        ...
        if current_metric is not None:
            ...
            if self.best_metric == float("inf") or self.best_metric == float("-inf"):
                logger.debug(
                    f"ModelCheckpoint: Initializing best {self.monitor} to {current_metric:.6f}"
                )
                ...
                if self.verbose:
                    logger.info(
                        f"Epoch {epoch + 1}: {self.monitor} improved from {self.best_metric:.6f} to {current_metric:.6f}."
                    )
                self.best_metric = current_metric
                self._best_epoch = epoch
                save_as_best = True
        ...
        if save_as_best:
            ...
            self._save_checkpoint(
                best_filepath, epoch, model, optimizer, current_metric
            )
            save_occurred = True
        ...
        if not self.save_best_only and save_this_epoch:
            try:
                epoch_filename = self.filename.format_map(
                    defaultdict(lambda: "NA", filepath_dict)
                )  # Use format_map with default
                ...
                if not (save_occurred and epoch_filepath == best_filepath):
                    self._save_checkpoint(
                        epoch_filepath, epoch, model, optimizer, current_metric
                    )
                    save_occurred = True
            except KeyError as e:
                logger.error(
                    f"Filename format error '{self.filename}'. Missing key: {e}. Available: {list(logs.keys())}"
                )
            except Exception as e:
                logger.error(f"Failed to save epoch checkpoint: {e}", exc_info=True)
        ...
        if self.save_last:
            ...
            if save_occurred:
                self.verbose = False
            self._save_checkpoint(
                last_filepath, epoch, model, optimizer, current_metric
            )
