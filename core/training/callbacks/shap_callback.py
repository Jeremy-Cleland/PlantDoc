        ...
        try:
            ...
            if self.dataset_split == "train" and 'train_loader' in logs:
                ...
                if data_module is not None:
                    if self.dataset_split == "train" and hasattr(data_module, 'train_dataloader'):
                        dataloader = data_module.train_dataloader()
                        logger.info("Using training dataset from data_module for SHAP analysis")
                    elif self.dataset_split == "val" and hasattr(data_module, 'val_dataloader'):
                        dataloader = data_module.val_dataloader()
                        logger.info("Using validation dataset from data_module for SHAP analysis")
                    elif hasattr(data_module, 'test_dataloader'):
                        dataloader = data_module.test_dataloader()
                        logger.info("Using test dataset from data_module for SHAP analysis")
                    else:
                        logger.error(f"Could not find {self.dataset_split} dataloader in data_module")
                else:
                    logger.error(f"No dataloader found for {self.dataset_split} split and no data_module available")
            ...
            # Try to get from the dataloader's dataset
                ...
                if hasattr(dataset, 'class_names'):
                    ...
                    logger.info(f"Using {len(class_names)} class names from dataset.classes")
                    
            if class_names is None:
                logger.warning("No class names found for SHAP analysis, visualization may be less interpretable")
