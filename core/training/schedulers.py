...
def get_scheduler(
    ...
    if scheduler_name == "step":
        ...
        logger.info(
            f"  ReduceLROnPlateau params: patience={patience}, factor={factor:.2f}, "
...
def get_scheduler_with_callback(
    ...
    from core.training.callbacks.lr_scheduler import (
    ...
    logger.info(
        f"  Creating LearningRateSchedulerCallback with mode='{step_mode}', monitor='{monitor}'"
    )
    callback = LearningRateSchedulerCallback(
        scheduler=scheduler, monitor=monitor, mode=step_mode, log_changes=log_changes
    )
