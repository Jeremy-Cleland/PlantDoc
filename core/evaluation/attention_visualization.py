...
class CBAMVisualizer:
    ...
    def __init__(
        ...
        logger.info(f"CBAMVisualizer initialized with {len(self.attention_modules)} attention modules")
    ...
    def _register_attention_hooks(self):
        ...
        for name, module in self.attention_modules.items():
            hook = module.register_forward_hook(
                lambda m, input, output, name=name: self._attention_hook(m, input, output, name)
            )
            self.hooks.append(hook)
            
        logger.info(f"Registered hooks for {len(self.attention_modules)} attention modules")
        ...
        if isinstance(image_input, str):
            ...
            if image_input.ndim == 4:  # (B, C, H, W)
                if image_input.size(0) != 1:
                    logger.warning(f"Batch size > 1 detected, using only the first image")
    ...
    def visualize_attention(
        ...
        for name, attention_map in attention_maps.items():
            ...
            if "_channel" in name:
                ...
                ax.bar(
                    ...
                    sorted_weights,
                    color=plt.cm.get_cmap(colormap)(np.linspace(0, 1, len(sorted_weights))),
                )
                ...
                # Save figure
                if output_dir:
                    fig.savefig(output_dir / f"{name}_channel_weights.png", bbox_inches="tight")
                ...
                for i in range(num_vis_channels):
                    if i < 3:  # Only process RGB channels for visualization
                        channel_attention_vis[:, :, i] = channel_weights[i] * original_image[:, :, i]
                
                # Normalize
                if channel_attention_vis.max() > 0:
                    channel_attention_vis = channel_attention_vis / channel_attention_vis.max()
                ...
                # Save figure
                if output_dir:
                    fig.savefig(output_dir / f"{name}_channel_effect.png", bbox_inches="tight")
        ...
        if spatial_maps:
            ...
            # Save figure
            if output_dir:
                fig.savefig(output_dir / "all_spatial_attention_maps.png", bbox_inches="tight")
...
def run_cbam_attention_visualization(
    ...
    for batch in dataloader:
        ...
        for i in range(len(images)):
            ...
            target_name = class_names[target] if class_names and target < len(class_names) else f"Class {target}"
            ...
            try:
                logger.info(f"Visualizing attention for sample {samples_processed+1}/{num_samples}: {target_name}")
                _ = visualizer.visualize_attention(
                    image, output_dir=sample_dir
                )
                ...
                if isinstance(image, torch.Tensor):
                    ...
                    for c in range(3):
                        img_np[:, :, c] = img_np[:, :, c] * visualizer.std[c] + visualizer.mean[c]
                    img_np = np.clip(img_np * 255, 0, 255).astype(np.uint8)
                    Image.fromarray(img_np).save(sample_dir / "original.png")
                    
                samples_processed += 1
            except Exception as e:
                logger.error(f"Error visualizing attention for sample {samples_processed}: {e}")
    
    # Clean up
    visualizer.cleanup()
    
    logger.info(f"CBAM attention visualization completed for {samples_processed} samples")
