...
class ConfusionFlow:
    ...
    def _calculate_flows(self) -> List[Dict[str, Union[int, float, str]]]:
        ...
        for true_idx in range(self.num_classes):
            for pred_idx in range(self.num_classes):
                ...
                flow_pct = flow_value / self.class_counts[true_idx] if self.class_counts[true_idx] > 0 else 0
    ...
    def plot(
        ...
        if not self.flows:
            ax.text(0.5, 0.5, "No significant misclassification flows to display",
        ...
        for i, (pos, err, count, name) in enumerate(zip(
            node_positions, self.error_rates, self.class_counts, self.class_names
        )):
            # Calculate node size based on count
            size = min_size + (max_size - min_size) * (count / max_count)
            
            # Draw node
            ax.scatter(pos[0], pos[1], s=size, 
                      color=node_colors[i], alpha=0.8, edgecolors='white', linewidth=1)
            ...
            ax.text(pos[0], pos[1], label, 
                   horizontalalignment='center', verticalalignment='center',
                   fontsize=label_fontsize, fontweight='bold', color=text_color,
                   path_effects=[
        ...
        for flow in sorted_flows:
            ...
            if show_values:
                ...
                ax.text(mid_x, mid_y, flow_text,
                       horizontalalignment='center', verticalalignment='center',
                       fontsize=max(8, min(12, label_fontsize-2)), fontweight='bold',
                       color='white', 
    ...
    def _get_node_positions(self, layout: str = 'circular') -> List[Tuple[float, float]]:
        """
